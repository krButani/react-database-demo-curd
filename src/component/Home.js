...
function Home() {
    ...
    function getSuccessMsg() {
        ...
        if(location.hasOwnProperty('state') && location.state != null && location.state.hasOwnProperty('succ') && location.state.succ.toString() !== "") {
            return <Box sx={{ marginX: 4, marginBottom: 2, }}>
    ...
    return (
        <Container>
            ...
            <Typography variant="h4" marginTop={2} marginX={4} gutterBottom>Display Post</Typography>
            {getSuccessMsg()}
                        ...
                        <TableBody>
                            {makeRows?.map((row,index) => (
                                <TableRow
                                    ...
                                    <TableCell>
                                        {row.body.substr(0,50)}...
                                    </TableCell>
                                   {/* <TableCell align="right"  onClick={handleCellClick}>*/}
                                    <TableCell align="right" >
                                        <Stack spacing={2} marginTop={3} direction="row">
                                            <Button variant="contained" id={row.id} size="small" color="warning" onClick={(e) => getURLCK(e,row.id)}>Update</Button>
                                            <Button variant="contained" id={row.id} disabled={row.disa} size="small" color="error" onClick={(e) => handleDeleteWithAxios(e,index,row.id)}>Delete</Button>
                                        </Stack>
                                    </TableCell>
                                </TableRow>
                            ))}
